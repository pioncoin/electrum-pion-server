How to run your own Electrum Pion server
========================================
...
Electrum Pion server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/pioncoin/electrum-pion-server/blob/master/HOWTO.md
...
document, we assume that user is called 'pion'). We also assume the
pion user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run piond and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit piond 
...
only leaving spare cycles for piond. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running piond and Electrum Pion server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `piond` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser pion --disabled-password
    $ sudo apt-get install git
    $ sudo su - pion
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/pion/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download piond

Recommend downloading latest version directly from Pioncoin.org

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - pion
    $ cd ~/src && wget https://github.com/pioncoin/pion/releases/download/0.12.3.1/pioncore-0.12.3-x86_64-linux-gnu.tar.gz
    $ sha256sum pioncore-0.12.3-x86_64-gnu.tar.gz | grep 39aca4b520cd4aaef1296462aabb8c76cbdce2ff6b356bd3fbac9748b62cf487
    $ tar xfz pioncore-0.12.3-x86_64-gnu.tar.gz
    $ cd pion-0.12.3/bin
    $ cp -a piond pion-cli pion-tx ~/bin

### Step 3. Configure and start piond

In order to allow Electrum Pion to "talk" to `piond`, we need to set up an RPC
username and password for `piond`. We will then start `piond` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.pion
    $ $EDITOR ~/.pion/pion.conf

Write this in `pion.conf`:
...
If you have an existing installation of piond and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ piond -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `piond`:

    $ piond

Allow some time to pass for `piond` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ pion-cli getblockchaininfo

Before starting the Electrum Pion server your piond should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start piond at boot
time, running as the 'pion' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Pion server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/pioncoin/electrum-pion-server.git
    $ cd electrum-pion-server
...
### Optional Step 5: Install Electrum Pion dependencies manually

Electrum Pion server depends on various standard Python libraries and leveldb. These will usually be
...
package manager if you don't want to use the install routine.
    ...
    $ git clone https://github.com/pioncoin/x16r_hash.git
    $ cd x16r_hash
    $ sudo python setup.py install
...
Electrum Pion server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Pion full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Pion server config.
...
### Step 9. Configure Electrum Pion server

Electrum Pion reads a config file (/etc/electrum-pion.conf) when starting up. This
file includes the database setup, piond RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-pion.conf
...
Electrum Pion server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-pion-server.
If you're using the user `pion` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "pion hard nofile 65536" >> /etc/security/limits.conf
     echo "pion soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Pion server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u pion -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close piond for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting piond (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Pion server

The magic moment has come: you can now start your Electrum Pion server as root (it will su to your unprivileged user):
...
If you want to stop Electrum Pion server, use the 'stop' command:
...
### Step 12. Test the Electrum Pion server
...
current server and connect to your new Electrum Pion server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
pions to confirm that everything is working properly.
...
If you're operating a public Electrum Pion server please subscribe
